...
<template>
  <div class="flex flex-col gap-5 max-w-2xl mx-auto">
    <NuxtLink :to="`/goals/${goalId}`" class="text-gray-500 dark:text-white/40 no-underline text-sm transition-colors hover:text-indigo-500">
      ← 目標に戻る
    </NuxtLink>

    <h1 class="m-0 text-2xl font-extrabold text-gray-900 dark:text-white/90 text-balance">目標を編集</h1>
    ...
    <div class="p-6 bg-white dark:bg-white/[0.04] border border-gray-200/80 dark:border-white/[0.08] rounded-2xl">
      <p class="m-0 mb-4 text-sm text-gray-500 dark:text-white/40">この目標を削除すると、紐づくマイルストーン・アウトプットもすべて削除されます。</p>
      <button
        type="button"
